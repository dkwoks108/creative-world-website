...
export default function AdminSidebar() {
    ...
    fetch("/api/site-data")
      ...
      .then((data) => {
        if (data.success && data.settings) {
          if (data.settings.company_name) setSiteName(data.settings.company_name);
          if (data.settings.logo_url) setLogoUrl(data.settings.logo_url);
        }
      })
      .catch((err) => console.error("Sidebar: site-data error:", err));
  ...
  return (
    <>
      ...
      >
        {/* Brand Header */}
        <div className="p-5 border-b border-[#1A2333]/80 bg-[#090D14]/50 backdrop-blur-md shrink-0">
        ...
        {/* Nav Items Grouped */}
        <nav className="flex-1 px-3 py-4 overflow-y-auto scrollbar-thin scrollbar-thumb-[#1E293B] space-y-6">
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive
                        ? "bg-gradient-to-r from-indigo-600 via-purple-600 to-cyan-500 text-white font-bold shadow-lg shadow-cyan-500/20"
        ...
        <div className="p-3 border-t border-[#1A2333] bg-[#0D121F]/40 space-y-2 shrink-0">
          {user && (
            <div className="flex items-center gap-3 p-2 rounded-xl bg-[#111622] border border-[#1E293B]">
              <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs shrink-0 shadow-md">
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-2 px-3 text-xs font-semibold text-slate-400 hover:text-red-400 bg-[#111622] hover:bg-red-500/10 border border-[#1E293B] hover:border-red-500/20 rounded-xl transition-all group"
          >
